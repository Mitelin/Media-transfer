...
def source_parent_cleanup_path(plan: MovePlan, mapping: dict[str, Any]) -> str | None:
    ...
    if not parent or not source_prefix or media_normpath(parent) == media_normpath(source_prefix):
        return None
    if not path_starts_with(parent, source_prefix):
        return None
    return parent
...
def log_source_parent_cleanup_plan(plan: MovePlan, mapping: dict[str, Any]) -> None:
    parent = source_parent_cleanup_path(plan, mapping)
    if parent:
        LOG.info("DRY RUN: would remove source parent folder if empty after move: %s", parent)
