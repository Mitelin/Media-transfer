...
Python finalizer pro Sonarr a Radarr post-import/post-upgrade workflow. První verze je bezpečně nastavená na `dry_run: true`, takže po doplnění endpointů a API klíčů nejdřív jen loguje vyhodnocení a plánované akce.
...
radarr_instances:
  movies:
    ...
    api_key: "PUT_RADARR_API_KEY_HERE"
    instance_type: "movie"
    maintenance_roots:
...
python3 scripts/sonarr_post_import_finalizer.py --config config/sonarr-finalizer.yml --instance movies --url-mode lan --test-api
```
...
python3 scripts/sonarr_post_import_finalizer.py --config config/sonarr-finalizer.yml --instance movies --url-mode lan --list-movies --root-prefix "/movies-en" --limit 20
```
...
Radarr dry-run simulace:

```bash
python3 scripts/sonarr_post_import_finalizer.py --config config/sonarr-finalizer.yml --instance movies --url-mode lan --movie-id 1 --movie-file-path "/movies-en/Test Movie (2026)/Test Movie.mkv" --enable-local-mounts
```
...
scripts/radarr_finalizer_movies.sh
```
...
/srv/scripts/sonarr/radarr_finalizer_movies.sh
...
chmod +x /srv/scripts/sonarr/radarr_finalizer_movies.sh
...
V Radarr Movies Custom Script nastav cestu:
